...
const thing = planets.map(element => {
...
const customerEmail = customers.map(email => {
    return email.contacts.email
});

const reduceEmail = customerEmail.reduce((total, emails) => {
...
// console.log(integers.sort((a, b) => b - a).filter(item => item < 19).map(item => (item * 1.5) - 1).reduce((total, current) => total += current))
...
const totalProfit = carData.map(item => item.gross_profit).reduce((total, current) => total += current)
...
let month = 0
let currentItem = 0
const highestMonth = carData.map(item => item.purchase_date).map(item => parseInt((item[5] + item[6]))).sort((a, b) => b - a).forEach(item => {
    if (currentItem === item || currentItem === 0) {
        counter += 1
        currentItem = item
    } else {
...
const salesPerson = carData.map(item => item.sales_agent.last_name).sort()

const salesPersonObject = {

}

salesPerson.forEach(person => {
    if (salesPersonObject.hasOwnProperty(person)) {
        salesPersonObject[person] += 1
    } else {
        salesPersonObject[person] = 1
    }
});
...
for (const key in salesPersonObject ) {
    if (salesPersonObject[key] > personCounter) {
        personCounter = salesPersonObject[key]
        bestPerson = key
    }
}
...
carData.forEach(person => {
    if (salesProfit.hasOwnProperty(person.sales_agent.last_name)) {
        salesProfit[person.sales_agent.last_name] += person.gross_profit
    } else {
        salesProfit[person.sales_agent.last_name] = person.gross_profit
    }
});
...
let highestProfit = 0
let mostProfitPerson = ""
for (const key in salesProfit ) {
    if (salesProfit[key] > highestProfit) {
        highestProfit = salesProfit[key]
        mostProfitPerson = key
    }
}

// console.log(mostProfitPerson)

const mostPopularModel = carData.map(item => item.vehicle.model)

const mostPopularModelObject = {

}
mostPopularModel.forEach(person => {
    if (mostPopularModelObject.hasOwnProperty(person)) {
        mostPopularModelObject[person] += 1
    } else {
        mostPopularModelObject[person] = 1
    }
});
...
let mostSold = 0
let highestSellingModel = ""
for (const key in mostPopularModelObject ) {
    if (mostPopularModelObject[key] > mostSold) {
        mostSold = mostPopularModelObject[key]
        highestSellingModel = key
    }
}

// console.log(mostPopularModelObject)
// console.log(highestSellingModel)

const bank = carData.map(item => item.credit.credit_provider)
...
bank.forEach(person => {
    if (creditObject.hasOwnProperty(person)) {
        creditObject[person] += 1
    } else {
        creditObject[person] = 1
    }
});
...
let bankCounter = 0
let mostUsedBank = ""
for (const key in creditObject ) {
    if (creditObject[key] > bankCounter) {
        bankCounter = creditObject[key]
        mostUsedBank = key
    }
}
